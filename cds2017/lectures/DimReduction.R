...
for (k in seq(10, 120, 10)) { 
  pciImage <- origImage.svd$u[,1:k] %*% diag(origImage.svd$d[1:k]) %*% t(origImage.svd$v[,1:k])
...
library(ggfortify)
autoplot(irisPCA, col = as.numeric(irisData$Species) + 1, loadings = TRUE, loadings.label = TRUE)
